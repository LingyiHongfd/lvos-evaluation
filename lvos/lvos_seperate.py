...
class LVOS(object):
    ...
    TASKS = ["semi-supervised", "unsupervised_multiple", "unsupervised_single"]
    ...
    def __init__(self, root, task="semi-supervised", subset="val", codalab=False):
        ...
        annotations_folder = (
            "Annotations"
            if task == "semi-supervised" or task == "unsupervised_single"
    ...
    def _check_directories(self):
        if not os.path.exists(self.root):
            raise FileNotFoundError(
                f"LVOS not found in the specified directory, download it from {self.DATASET_WEB}"
            )
    ...
    def get_mask(self, sequence, frame, target_obj=None):
        masks = np.array(
            Image.open(os.path.join(self.mask_path, sequence, frame + ".png"))
        )
