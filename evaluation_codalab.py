...
dataset_eval = LVOSEvaluation_MP(
    lvos_root=gt_path, gt_set=gt_set, task=task, codalab=True, mp_procs=4
)
...
if gt_set == "val" or gt_set == "train" or gt_set == "test":
    ...
    table_g = pandas.DataFrame(
        data=np.reshape(g_res, [1, len(g_res)]), columns=g_measures
    )
